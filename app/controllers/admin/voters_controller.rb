class Admin::VotersController < ApplicationController
  ...
  def search
    ...
    @party_breakdown = @voters.select("party, count(*) as voter_count").group(:party).order("voter_count desc")
    @precinct_breakdown = @voters.select("precinct_code, count(*) as voter_count").group(:precinct_code).order("voter_count desc")
