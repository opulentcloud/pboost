class Customers::VoterVerifyController < AuthenticatedUsersController
  ...
  def search
    ...
    voter_dob = Date.parse("#{Date.today.year}-#{voter_params[:birth_month]}-#{voter_params[:birth_day]}") rescue ""
...
private
  def voter_params
    @voter_params ||= params.require(:voter).permit(:first_name, :last_name, :street_no, :dob, :search_index, :search_index2, :birth_month, :birth_day)
  end
end
