...
class WalksheetReportPdf < Prawn::Document
  ...
  def initialize(voter_ids, default_prawn_options = {})
    ...
    Voter.includes(:address).where(id: voter_ids).order("addresses.street_name, addresses.is_odd, addresses.street_no, addresses.apt_no").each_with_index do |voter, index|
      first = true
